...
## Branch frozen for Webbinar 6

This Branch is a frozen version for JWebbinar 6 "NIRCam and MIRI point-source imaging". If you want a version of the materials that is being maintained, see the `main` branch of this repository.
...
`git checkout webbinar6`  
`cd pointsource_imaging`  

Alternatively, you can manually download the individual notebooks by clicking on the pointsource_imaging folder, then click on each file, and right-click on "Raw" to select "Save link as".
...
Or if you are using anaconda, you first create an environment with the name of your choice (we use `jwebbinar6` for this example):  
`conda create -n jwebbinar6 python=3.8`  
`conda activate jwebbinar6`  
`pip install -r requirements.txt`  
`pip install jupyter`  
